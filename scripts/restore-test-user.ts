...
async function main() {
  ...
  if (existingUser) {
    ...
    console.log(
      "💳 Suscripción:",
      existingUser.tenant.subscription?.planType,
      existingUser.tenant.subscription?.status
    );
    console.log(
      "📅 Trial hasta:",
      existingUser.tenant.subscription?.trialEndsAt?.toLocaleDateString()
    );
    return;
  }
