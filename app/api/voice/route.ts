...
async function processVoiceCommand(transcript: string, userId: string) {
  const lowerTranscript = transcript.toLowerCase();

  // Comandos de creación de paciente
  if (lowerTranscript.includes('crear') && lowerTranscript.includes('paciente')) {
    return await handleCreatePatient(transcript, userId);
  }
  ...
  if (lowerTranscript.includes('buscar') || lowerTranscript.includes('encontrar')) {
    return await handleSearch(transcript, userId);
  }

  // Comando de ver agenda
  if (lowerTranscript.includes('agenda') || lowerTranscript.includes('citas del día')) {
    return await handleViewSchedule(transcript, userId);
  }

  return {
    success: false,
    message: 'No entendí el comando. Intenta con: "crear paciente", "agendar cita", "ver agenda", etc.',
    suggestedActions: [
...
async function handleCreatePatient(transcript: string, userId: string) {
  ...
  try {
    ...
    const createResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/patients`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${userId}`, // En producción, usar token real
      },
...
async function handleSearch(transcript: string, userId: string) {
  ...
  const searchTerm = transcript.replace(/buscar|encontrar|paciente/gi, '').trim();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await processVoiceCommand(transcript, user.userId);
